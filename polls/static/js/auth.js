var Auth = {
	vars: {
		...
		stu_register_link: document.querySelector('.stu_register-link'),
		com_register_link: document.querySelector('.com_register-link'),
		password_group: document.querySelector('.password-group'),
		password_group_height: 0,
		lowin_stu_register: document.querySelector('.lowin-stu_register'),
		lowin_com_register: document.querySelector('.lowin-com_register'),
	...
	stu_register(e) {
		...
		Auth.vars.lowin_stu_register.style.display = 'block';
		Auth.vars.lowin_stu_register.className += ' lowin-animated-flip';

		Auth.setHeight(Auth.vars.lowin_stu_register.offsetHeight + Auth.vars.lowin_footer.offsetHeight);
	...
	com_register(e) {
		...
		Auth.vars.lowin_com_register.style.display = 'block';
		Auth.vars.lowin_com_register.className += ' lowin-animated-flip';

		Auth.setHeight(Auth.vars.lowin_com_register.offsetHeight + Auth.vars.lowin_footer.offsetHeight);

		e.preventDefault();
	},
	login(e) {
		Auth.vars.lowin_stu_register.classList.remove('lowin-animated-flip');
		Auth.vars.lowin_stu_register.className += ' lowin-animated-flipback';
		...
		setTimeout(() => {
			Auth.vars.lowin_stu_register.style.display = 'none';
		}, 500);
		
		setTimeout(() => {
			Auth.vars.lowin_stu_register.classList.remove('lowin-animated-flipback');
	...
	init(option) {
		...
		Auth.vars.stu_register_link.addEventListener("click", (e) => {
			Auth.brand();
			Auth.stu_register(e);
		});

		Auth.vars.com_register_link.addEventListener("click", (e) => {
			Auth.brand();
			Auth.com_register(e);
		});
